    ...
    def read_bytes_term(self, term, include_term, consume_term, eos_error):
        ...
        while True:
            ...
            if c == b"":
                if eos_error:
                    raise Exception(
                        "end of stream reached, but no terminator %d found" % (term,)
                    )
    ...
    def process_xor_many(data, key):
        if PY2:
            return bytes(
                bytearray(
                    a ^ b
                    for a, b in zip(bytearray(data), itertools.cycle(bytearray(key)))
                )
            )
...
class ValidationFailedError(KaitaiStructError):
    ...
    def __init__(self, msg, io, src_path):
        super(ValidationFailedError, self).__init__(
            "at pos %d: validation failed: %s" % (io.pos(), msg), src_path
        )
        self.io = io
