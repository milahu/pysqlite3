...
class Sqlite3(KaitaiStruct):
    ...
    def _read(self):
        ...
        self.write_version = KaitaiStream.resolve_enum(
            Sqlite3.Versions, self._io.read_u1()
        )
        ...
        self.text_encoding = KaitaiStream.resolve_enum(
            Sqlite3.Encodings, self._io.read_u4be()
        )
    ...
    class CellPayload(KaitaiStruct):
        ...
        def _read(self):
            ...
            self._raw_column_serials = self._io.read_bytes(
                (self.len_header_and_len.value - 1)
            )
            _io__raw_column_serials = KaitaiStream(BytesIO(self._raw_column_serials))
            self.column_serials = Sqlite3.Serials(
                _io__raw_column_serials, self, self._root
            )
            self.column_contents = []
            for i in range(len(self.column_serials.entries)):
                self.column_contents.append(
                    Sqlite3.ColumnContent(
                        self.column_serials.entries[i], self._io, self, self._root
                    )
                )
